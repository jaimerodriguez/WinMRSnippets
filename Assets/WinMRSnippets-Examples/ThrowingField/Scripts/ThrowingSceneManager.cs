...
public class ThrowingSceneManager : MonoBehaviour
{
    ...
    private void InteractionManager_InteractionSourceReleased(InteractionSourceReleasedEventArgs args)
    {
        if (args.pressType == InteractionSourcePressType.Grasp || args.pressType == InteractionSourcePressType.Select)
        {
            ...
            if (devices.ContainsKey(id))
            {
                ...
                args.state.sourcePose.TryGetVelocity(out velocity);
                rigidbody.AddForce(velocity * 1f, ForceMode.Impulse);
                rigidbody.velocity = velocity;

                //Vector3 forward;
                //args.state.sourcePose.TryGetForward(out forward, InteractionSourceNode.Grip);
                //rigidbody.angularVelocity = forward * 10f;
                rigidbody.maxAngularVelocity = 25f;
                rigidbody.AddRelativeTorque(new Vector3(0f, 0f, 100f), ForceMode.Impulse);
    ...
    private void SetTransform(Transform t, Vector3 position, Quaternion rotation)
    {
            ...
            float.IsNaN(rotation.w) || float.IsNaN(rotation.x) || float.IsNaN(rotation.y) || float.IsNaN(rotation.z)))
        {
            ...
            // Adjust tilt of football in hand so nose isn't pointed down
            Vector3 eul = rotation.eulerAngles;
            t.localRotation = Quaternion.Euler(eul.x + 40, eul.y, eul.z);
        }
    }
